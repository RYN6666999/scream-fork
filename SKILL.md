---
name: troubleshooter
description: Use when build fail, test fail, connection error, or agent behaves unexpectedly.
---
...
| 你心裡會想 | 事實 |
|-----------|------|
| 「錯誤訊息看起來很眼熟，我知道怎麼修」 | 看完整錯誤，不要只看最後一行。熟悉的錯誤可能是新的 root cause。 |
| 「先改 code 再確認問題」 | 先 Step 1-4 排查流程，不要跳過直達 fix。 |
| 「這次先跳過排查直接試我知道的解法」 | 3 步沒過才升級。先走標準流程。 |
| 「這個錯誤跟上次一樣，用上次的解法就行」 | 每次排查都從 step 1 開始。走捷徑會 miss 真正的 root cause。 |
